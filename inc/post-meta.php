	...
	function csco_get_meta_author( $tag = 'span', $compact = true ) {
		...
		if ( $authors ) {
			...
			foreach ( $authors as & $author ) {
				...
				$output .= $counter > 0 ? sprintf( '<span class="sep">%s</span>', esc_html__( 'and', 'spotlight' ) ) : '';
				...
				$output .= sprintf( '<span class="author"><a class="url fn n" href="%1$s" title="%2$s">%3$s</a></span>',
					...
					esc_attr( sprintf( __( 'View all posts by %s', 'spotlight' ), $display_name ) ),
					$display_name
				);
...
if ( ! function_exists( 'csco_get_meta_comments' ) ) {
	...
	function csco_get_meta_comments( $tag = 'span', $compact = false ) {
		...
		if ( true === $compact ) {
			...
			ob_start();
			comments_popup_link( esc_html__( 'No comments', 'spotlight' ), esc_html__( 'One comment', 'spotlight' ), '% ' . esc_html__( 'comments', 'spotlight' ), 'comments-link', '' );
			$output .= ob_get_clean();
		}
...
if ( ! function_exists( 'csco_get_meta_reading_time' ) ) {
	...
	function csco_get_meta_reading_time( $tag = 'span', $compact = false ) {
		...
		if ( true === $compact ) {
			$output .= intval( $reading_time ) . ' ' . esc_html__( 'min', 'spotlight' );
		} else {
			/* translators: %s number of minutes */
			$output .= esc_html( sprintf( _n( '%s minute read', '%s minute read', $reading_time, 'spotlight' ), $reading_time ) );
		}
...
if ( ! function_exists( 'csco_get_meta_views' ) ) {
	...
	function csco_get_meta_views( $tag = 'span', $compact = false ) {
		...
		if ( true === $compact ) {
			...
			if ( $views > 1000 ) {
				$output .= $views_rounded . ' ' . esc_html__( 'views', 'spotlight' );
			} else {
				/* translators: %s number of post views */
				$output .= esc_html( sprintf( _n( '%s view', '%s views', $views, 'spotlight' ), $views ) );
			}
		}
			...
			<span class="total">
				...
				<span class="total-number">
					...
					if ( true === $compact ) {
						...
						if ( $shares > 1000 ) {
							echo esc_html( $shares_rounded ) . ' ' . esc_html__( 'shares', 'spotlight' );
						} else {
							/* translators: %s number of post views */
							echo esc_html( sprintf( _n( '%s share', '%s shares', $shares, 'spotlight' ), $shares ) );
						}
					}
					?>
				</span>
