...
function csco_get_custom_content_locations() {
	$content = array(
		array(
			'slug'     => 'header',
			'name'     => esc_html__( 'Header', 'spotlight' ),
			'template' => array( 'home', 'front_page', 'single', 'page', 'archive' ),
		),
		array(
			'slug'     => 'site_content',
			'name'     => esc_html__( 'Site Content', 'spotlight' ),
			'template' => array( 'home', 'front_page', 'single', 'page', 'archive' ),
		),
		array(
			'slug'     => 'main',
			'name'     => esc_html__( 'Main', 'spotlight' ),
			'template' => array( 'home', 'front_page', 'single', 'page', 'archive' ),
		),
		array(
			'slug'     => 'homepage_posts',
			'name'     => esc_html__( 'Featured Posts', 'spotlight' ),
			'template' => array( 'home', 'front_page' ),
		),
		array(
			'slug'     => 'post',
			'name'     => esc_html__( 'Post', 'spotlight' ),
			'template' => array( 'single' ),
		),
		array(
			'slug'     => 'post_content',
			'name'     => esc_html__( 'Post Content', 'spotlight' ),
			'template' => array( 'single' ),
		),
		array(
			'slug'     => 'page',
			'name'     => esc_html__( 'Page', 'spotlight' ),
			'template' => array( 'page' ),
		),
		array(
			'slug'     => 'page_content',
			'name'     => esc_html__( 'Page Content', 'spotlight' ),
			'template' => array( 'page' ),
		),
		array(
			'slug'     => 'author',
			'name'     => esc_html__( 'Post Author', 'spotlight' ),
			'template' => array( 'single' ),
		),
		array(
			'slug'     => 'pagination',
			'name'     => esc_html__( 'Post Pagination', 'spotlight' ),
			'template' => array( 'single' ),
		),
		array(
			'slug'     => 'comments',
			'name'     => esc_html__( 'Comments', 'spotlight' ),
...
function csco_get_custom_content_pages() {
	$pages = array(
		'home'       => esc_html__( 'Homepage', 'spotlight' ),
		'front_page' => esc_html__( 'Front Page', 'spotlight' ),
		'single'     => esc_html__( 'Post', 'spotlight' ),
		'page'       => esc_html__( 'Page', 'spotlight' ),
		'archive'    => esc_html__( 'Archive', 'spotlight' ),
	);
	return $pages;
}
...
function csco_init_custom_content() {
	...
	CSCO_Kirki::add_panel(
		'custom_content', array(
			'title'    => esc_html__( 'Additional Content', 'spotlight' ),
			'priority' => 200,
		)
	);
	...
	CSCO_Kirki::add_field(
		'csco_theme_mod', array(
			...
			'label'       => esc_html__( 'Load Google AdSense Scripts', 'spotlight' ),
			'description' => esc_html__( 'Enable this if you\'re using Google AdSense.', 'spotlight' ),
			...
			if ( in_array( $page_slug, $location['template'], true ) ) {
				...
				CSCO_Kirki::add_field(
					'csco_theme_mod', array(
						...
						'label'             => esc_html__( 'Before', 'spotlight' ) . ' ' . $location['name'],
				...
				CSCO_Kirki::add_field(
					'csco_theme_mod', array(
						...
						'label'             => esc_html__( 'After', 'spotlight' ) . ' ' . $location['name'],
						'section'           => 'custom_content_' . $page_slug,
						'default'           => '',
						'priority'          => 10,
						'sanitize_callback' => 'csco_unsanitize',
