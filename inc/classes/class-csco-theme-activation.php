...
if ( ! class_exists( 'CSCO_Theme_Activation' ) ) {
	...
	class CSCO_Theme_Activation {
		...
		public function register_options_page() {
			add_theme_page( esc_html__( 'Theme License', 'spotlight' ), esc_html__( 'Theme License', 'spotlight' ), 'manage_options', 'csco-activation', array( $this, 'settings_page' ) );
		}
		...
		public function settings_page() {
			...
			if ( ! current_user_can( 'manage_options' ) ) {
				wp_die( esc_html__( 'You do not have sufficient rights to view this page.', 'spotlight' ) );
			}
			?>
				<div class="wrap">
					<h1><?php esc_html_e( 'Theme License', 'spotlight' ); ?></h1>
					...
					<div id="poststuff">
						...
						<?php if ( $status && ! get_option( $this->setting_subscribe() ) && 'dev' !== csco_get_license_data( 'item_id' ) ) { ?>
							<div class="postbox">
								...
								<h2 class="hndle"><span><?php esc_html_e( 'Updates', 'spotlight' ); ?></span></h2>

								<div class="inside">
									<p style="font-size: 14px;margin-bottom: 0;"><?php esc_html_e( 'We set a special price for all new themes for just a few days. Get notified of all introductory, flash and seasonal sales by signing up to our updates.', 'spotlight' ); ?></p>
									...
									<form method="post" style="max-width:864px;">
										...
										<table class="form-table">
											<!-- Email Address -->
											<tr>
												<th scope="row"><?php esc_html_e( 'Email Address', 'spotlight' ); ?></label></th>
												<td>
											...
											<tr>
												...
												<div style="display:flex;">
													...
													<label for="newsletter">
														<p><?php esc_html_e( 'By checking this box you agree to our', 'spotlight' ); ?> <a href="https://codesupply.co/terms-and-conditions/" target="_blank"><?php esc_html_e( 'Terms and Conditions', 'spotlight' ); ?></a> <?php esc_html_e( 'and', 'spotlight' ); ?> <a href="https://codesupply.co/privacy-policy/" target="_blank"><?php esc_html_e( 'Privacy Policy', 'spotlight' ); ?></a>.</p>

														<p class="description"><?php esc_html_e( 'You may opt-out any time by clicking the unsubscribe link in the footer of any email you receice from us, or by contacting us at', 'spotlight' ); ?> <a target="_blank" href="mailto:support@codesupply.co"><?php esc_html_e( 'support@codesupply.co', 'spotlight' ); ?></a>.</p>
													</label>
													</div>
												</td>
											</tr>
											...
											<tr>
												<th scope="row"></th>
												<td>
													<button name="action" value="subscribe" type="submit" class="button button-primary button-large" id="publish"><?php esc_html_e( 'Subscribe', 'spotlight' ); ?></button>
												</td>
											</tr>
										</table>
									</form>
						...
						<div class="postbox">
							...
							<h2 class="hndle">
								<span>
									<?php if ( $status ) { ?>
										<?php esc_html_e( 'License Information', 'spotlight' ); ?>
									<?php } else { ?>
										<?php esc_html_e( 'License Activation', 'spotlight' ); ?>
									<?php } ?>
								</span>
							</h2>

							<div class="inside">
								<p style="font-size: 14px;margin-bottom: 0;" <?php echo esc_attr( $status ? 'hidden' : null ); ?>"><?php esc_html_e( 'To unlock demo content, please enter your purchase code below. If you don’t have a purchase code, please purchase the theme on', 'spotlight' ); ?> <a target="_blank" href="https://themeforest.net/user/codesupplyco/portfolio?ref=codesupplyco"><?php esc_html_e( 'ThemeForest', 'spotlight' ); ?></a>.</p>
								...
								<?php if ( $status ) { ?>
									<form method="post">
										...
										<table class="form-table">
											<tr>
												<th scope="row"><?php esc_html_e( 'Active Websites', 'spotlight' ); ?></label></th>
												<td>
													<?php
													...
													if ( $history ) {
														...
														<p>
															...
															<button class="button" type="submit" name="action" value="check"><?php esc_html_e( 'Check Again', 'spotlight' ); ?></button>
														</p>
														<?php
													} elseif ( 'dev' === csco_get_license_data( 'item_id' ) ) {
														esc_html_e( 'Development Mode', 'spotlight' );
													}
													?>
												</td>
											</tr>
								...
								<form method="post">
									...
									<table class="form-table">
										...
										<tr class="<?php echo esc_attr( $status ? 'hidden' : null ); ?>">
											<th scope="row"><?php esc_html_e( 'Purchase Code', 'spotlight' ); ?></label></th>
											<td>
										...
										<!-- Purchase Date -->
										<tr class="<?php echo esc_attr( ! $status ? 'hidden' : null ); ?>">
											<th scope="row"><?php esc_html_e( 'Purchase Date', 'spotlight' ); ?></label></th>
											<td>
										...
										?>
											<tr class="<?php echo esc_attr( ! $status ? 'hidden' : null ); ?>">
												<th scope="row"><?php esc_html_e( 'Supported Until', 'spotlight' ); ?></label></th>
												<td>
													<span style="color:red">
														<?php echo esc_html( date( 'F d, Y', strtotime( $supported_until ) ) ); ?>

														<?php esc_html_e( ' (Expired)', 'spotlight' ); ?>
													</span>
													<p class="description"><?php esc_html_e( 'Please renew your item support for any support requests. See', 'spotlight' ); ?> <a href="https://help.market.envato.com/hc/en-us/articles/207886473-Extending-and-Renewing-Item-Support" target="_blank"><?php esc_html_e( 'this document', 'spotlight' ); ?></a> <?php esc_html_e( 'for more details.', 'spotlight' ); ?></p>
												</td>
											</tr>
										<?php } ?>
										<!-- Submitbox -->
										<tr>
											<th scope="row">
												<?php
												if ( $status ) {
													esc_html_e( 'Deactivation', 'spotlight' );
												}
												?>
											</th>
											<td>
												<?php if ( $status ) { ?>
													<button name="action" value="deactivate" type="submit" class="button button-primary button-large" id="publish"><?php esc_html_e( 'Deactivate License', 'spotlight' ); ?></button>
												<?php } ?>

												<?php if ( ! $status ) { ?>
													<button name="action" value="activate" type="submit" class="button button-primary button-large" id="publish"><?php esc_html_e( 'Activate License', 'spotlight' ); ?></button>
												<?php } ?>
											</td>
										</tr>
									</table>
		...
		public function trigger_license() {
			...
			if ( isset( $_POST['code'] ) && $_POST['code'] ) { // Input var ok; sanitization ok.
				...
				$this->msg = $this->set_message( esc_html__( 'Purchase code not entered.', 'spotlight' ) );
				return;
			}
			...
			if ( 'subscribe' === $action ) {
				if ( isset( $_POST['email'] ) && $_POST['email'] ) { // Input var ok; sanitization ok.
					...
					$this->msg = $this->set_message( esc_html__( 'Email address is considered invalid.', 'spotlight' ) );
					return;
				}

				if ( ! isset( $_POST['newsletter'] ) ) { // Input var ok; sanitization ok.
					$this->msg = $this->set_message( esc_html__( 'Please agree to our terms and conditions.', 'spotlight' ) );
					return;
				}
			}
			...
			if ( is_wp_error( $response ) ) {
				$this->msg = $this->set_message( esc_html__( 'No connection to the server, try another time, or contact support.', 'spotlight' ) );
				return;
			}
			...
			if ( ! isset( $data['message'] ) ) {
				$this->msg = $this->set_message( esc_html__( 'Could not receive data from the server, try another time, or contact support.', 'spotlight' ) );
			} else {
		...
		public function display_license_notice() {
			...
			if ( strtotime( $supported_until ) < strtotime( 'now' ) ) {
				...
				if ( ( ! get_transient( $transient_name ) && $dismissible ) || ! $dismissible ) {
					...
					<div class="csco-notice notice notice-warning <?php echo esc_attr( $dismissible ); ?>" data-notice="<?php echo esc_attr( $transient_name ); ?>">
						<p><strong><?php esc_html_e( 'Support expired.', 'spotlight' ); ?></strong>
						<?php
							// Translators: theme name and link activation.
							echo wp_kses( sprintf( __( 'Your support for the %1$s theme has expired. Please %2$s for any support requests.', 'spotlight' ), $this->theme_name, '<a href="https://help.market.envato.com/hc/en-us/articles/207886473-Extending-and-Renewing-Item-Support" target="_blank">' . __( 'renew your support license', 'spotlight' ) . '</a>' ), 'post' );
						?>
						</p>
					</div>
					<?php
			...
			if ( count( $actived ) > $count ) {
				...
				?>
					<div class="csco-notice notice notice-warning <?php echo esc_attr( $dismissible ); ?>" data-notice="<?php echo esc_attr( $transient_name ); ?>">
						<p><strong><?php esc_html_e( 'Multiple active websites detected.', 'spotlight' ); ?></strong> <?php esc_html_e( 'Looks like you’re using the same theme license on multiple websites. A website theme can only be customized to create one customized website according to the ThemeForest license terms. If you want to create a second website from the same theme, you’ll need to', 'spotlight' ); ?> <a href="<?php esc_url( admin_url( '/themes.php?page=csco-activation' ) ); ?>"><?php esc_html_e( 'purchase another license', 'spotlight' ); ?></a>.</p>
					</div>
				<?php
				}
			}
		...
		<div class="notice notice-warning notice-alt">
			<p>
				...
				if ( is_customize_preview() ) {
					...
					echo wp_kses( sprintf( __( 'Please %1$s to unlock theme demos.', 'spotlight' ), '<a class="button-link" href="' . admin_url( '/themes.php?page=csco-activation' ) . '">' . __( 'activate your license', 'spotlight' ) . '</a>' ), 'post' );
				} else {
					// Translators: link activation.
					echo wp_kses( sprintf( __( 'Please %1$s to unlock demo content.', 'spotlight' ), '<a class="button-link" href="' . admin_url( '/themes.php?page=csco-activation' ) . '">' . __( 'activate your license', 'spotlight' ) . '</a>' ), 'post' );
				}
				?>
			</p>
		</div>
