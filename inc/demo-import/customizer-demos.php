...
function register_csco_demos_control() {
	...
	final class CSCO_Demos_Control extends WP_Customize_Control {
		...
		protected function render_content() {
			?>
				<h3><?php esc_html_e( 'Reset', 'spotlight' ); ?></h3>

				<p><?php esc_html_e( 'Click the button to reset the customization settings for this theme.', 'spotlight' ); ?></p>

				<p>
					<input type="submit" name="csco-demos-reset" class="button-secondary button" value="<?php esc_html_e( 'Reset', 'spotlight' ); ?>">
				...
				<?php if ( isset( $demos['demos'] ) && $demos['demos'] ) { ?>
					<h3><?php esc_html_e( 'Select Demo', 'spotlight' ); ?></h3>
									...
									<div class="theme csco-theme-demo" style="width: 100%;" data-demo="<?php echo esc_attr( $slug ); ?>">
										...
										<div class="theme-id-container">
											...
											<div class="theme-actions">
												...
												<a class="button button-primary csco-demo-import" href="#"><?php esc_html_e( 'Activate', 'spotlight' ); ?></a>
											</div>
										</div>
									</div><br>
								<?php
...
class CSCO_Demos_Core {
	...
	public function register( $wp_customize ) {
		...
		$wp_customize->add_section(
			'csco-section', array(
				'title'    => __( 'Theme Demos', 'spotlight' ),
				'priority' => 2,
			)
		);
		...
		<script>
			jQuery(function ($) {
				$('body').on('click', '.csco-demo-import', function (event) {
					...
					var r = confirm( "<?php esc_html_e( 'Warning: Activating a demo will reset all current customizations.', 'spotlight' ); ?>" );
						...
						try {
							...
							if( typeof info.success != 'undefined' && info.success == true ){
								...
								if( typeof info.data != 'undefined' ){
									...
									alert( '<?php esc_html_e( 'Server error!', 'spotlight' ); ?>' );
								}
							}
						} catch (e) {
							alert( response );
		...
		<script>
			jQuery(function ($) {
				$('body').on('click', 'input[name="csco-demos-reset"]', function (event) {
					...
					var r = confirm( "<?php esc_html_e( 'Warning: This action will reset all current customizations.', 'spotlight' ); ?>" );
						...
						try {
							...
							if( typeof info.success != 'undefined' && info.success == true ){
								...
								if( typeof info.data != 'undefined' ){
									...
									alert( '<?php esc_html_e( 'Error server!', 'spotlight' ); ?>' );
								}
							}
						} catch (e) {
							alert( response );
